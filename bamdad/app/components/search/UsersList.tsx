...
export default function UsersList({searchClick=false}:{searchClick?:boolean}) {
  const { t } = useTranslation();
  const data = useSelector((state: RootState) => state.Search.users)
  return (
    <>
      {data.length > 0 &&
        <>
          <div className="flex flex-row mb-2 mt-5">
            <div className='flex-grow'><b className='text-[var(--black)]' >{t("accounts")}</b></div>
              ...
              <Link href={`/business/${item.id}`}>
                <div className="flex flex-row p-3 items-center gap-x-4" key={item.id}>
                  ...
                  <div className="flex-grow  flex flex-col gap-y-2">
                    ...
                    <div className="text-[var(--gray)] text-sm"><p>{item.about?.substring(0, 70)}...</p></div>
