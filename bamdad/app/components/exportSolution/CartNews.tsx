...
export default function CartNews({ data }: { data: ExportSolutionProps }) {
    let image1 = data.attach.length > 0 ? data.attach[0].path : "";
    let image2 = data.attach.length > 1 ? data.attach[1].path : "";
    let image3 = data.attach.length > 2 ? data.attach[2].path : "";
    return (
        <div className="bg-[var(--grayHashtag)] rounded-2xl p-4  mb-3" >
            <Link href={"/exportSolution/" + data.id}>
                <div className='flex flex-col gap-y-4'>
                    <div className=''>
                        <p className="text-[var(--black)] text-sm font-bold">
                            {data.title ?? "استفاده حداکثری از فضای باز برای یک باغ سبزی فراوان و زیبا در کشور استوایی"}
                        </p>
                    </div>
