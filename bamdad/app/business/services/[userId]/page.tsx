import { UsersApi, UsersServiceListApi } from '@/app/redux/ApiCall/ApiAddress';
...
export default async function page({ params }: { params: { userId: string } }) {
    ...
    const res2 = await fetch(baseURL + UsersServiceListApi + `/${params.userId}`, { cache: 'no-store', next: { revalidate: 5 } });
    const DataDetail2 = await res2.json()
    return (
        <LayoutMobileInnerPage urlBack='/business'  type='more'>
            <Services
                userDatas={DataDetail.data}
                serviceData={DataDetail2.data.data}
                haveAction={false}
